...
import com.kochun.wxmp.common.utils.HexConvertUtil;
...
//    }
    ...
    public static String enCrypto(String str) {
        try {
            ...
            secureRandom.setSeed(Base64ConvertUtil.decode(encryptSalt).getBytes());
    ...
    public static String deCrypto(String str) {
        try {
            ...
            // 将私钥encryptAESKey先Base64解密后转换为byte[]数组按128位初始化
            SecureRandom secureRandom = SecureRandom.getInstance("SHA1PRNG");
            secureRandom.setSeed(Base64ConvertUtil.decode("F12839WhsnnEV$#23b").getBytes());
            keygen.init(128, secureRandom);
            // SecretKey 负责保存对称密钥 生成密钥
            SecretKey desKey = keygen.generateKey();
            // 生成Cipher对象,指定其支持的AES算法，Cipher负责完成加密或解密工作
